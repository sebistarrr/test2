 ...
 *   2. chaque fiche du roster porte **tous ses champs anglais**. Le repli de
 *      `label()` (`nameRef ?? name`) évite le plantage, pas l'incohérence : un
 *      champ oublié fait apparaître une ligne française au milieu d'un écran
 *      anglais, et ça ne se voit qu'en survolant la bonne carte.
 *
...
const rapport = await page.evaluate(async () => {
  ...
  const { ELEMENTS, ROSTER } = await import('/src/data/elements.js');
  ...
  return { nbCles: cles.length, nbFiches: ROSTER.length, lignes, erreurs };
});
...
if (rapport.erreurs.length) {
  ...
  console.log(`\nOK — les deux tables concordent, les ${rapport.nbFiches} fiches sont complètes`);
}
