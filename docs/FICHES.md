...
| La norme passe à 200 PV, le Golem à 400 (historique) | 3171 |
| Neon Shadow supprimé, la norme redescend à 100 PV | 3226 |
| Les dégâts de tous les combattants, divisés par deux | 3294 |
| Rééquilibrage confiné au Golem et au Ronin | 3404 |
| Nerf confiné au Shinobi et au Pistolero, les deux qui dominaient | 3491 |
| Le son de chacun | 3541 |
| Équilibrage du roster | 3659 |
| Règles communes (moteur) | 3757 |
| Comment les mesures ont été prises | 3781 |
...
| Faisceau | **Relevé au profil transversal de la maquette fournie**, et non composé : moyenne des colonnes sur la moitié droite de l'image, puis lecture des ruptures de teinte, positions normalisées sur le bord extérieur de l'encre brûlée — qui est aussi `halfWidth`, donc sur ce que le rayon blesse. Sept bandes : `core` crème au centre (0 → 0,11), `light` (→ 0,27), `body` (→ 0,62), `shadow` (→ 0,83), `edge` (→ 1,00), puis **deux bandes de lueur au-delà de 1** (1,18 et 1,42). **Ce que l'ancien faisceau n'avait pas, et c'est tout le changement : un liseré clair *à l'extérieur* de l'encre.** Les cinq bandes d'avant allaient du sombre au clair de façon monotone, donc le rayon se terminait sur son trait le plus sombre et se posait comme une barre peinte ; la maquette **rallume** au-delà du contour, et c'est ce halo qui le fait rayonner. Les bandes au-delà de 1 sont du **décor, pas de la géométrie** : `halfWidth` borne les dégâts, donc l'encre brûlée marque exactement le bord de ce qui blesse | relevé sur la maquette |
| Filaments et braises | Les **zigzags** qui courent dans le faisceau, et les **braises** qui le remontent depuis l'émetteur — la seule chose qui dise qu'il *coule* plutôt qu'il n'est allumé, un rayon de 2,5 s étant sinon une image fixe. Longueurs en multiples de `halfWidth`, donc tout suit si on élargit. **Aucun tirage** : les sommets sont posés aux pointes (échantillonner une onde triangulaire ferait *respirer* l'amplitude au lieu de la faire défiler) et l'ensemble glisse avec le temps | calé sur la maquette |
