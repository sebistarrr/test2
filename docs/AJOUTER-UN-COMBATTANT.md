...
Éprouvé sur l'ajout du Druide : quatre fichiers créés, neuf lignes ajoutées dans
...
Ça se fait, et ça ne se recopie pas. Le Druide porte les deux pouvoirs de la
...
- **les valeurs dérivées de `f.stacks`.** Chez la Plante, les dégâts du bulbe
  suivent la pile ; chez le Druide la pile est la cadence de tir. Les fonctions
...
- **les littéraux du module d'origine.** `plant.js` codait en dur la clé de
  sprite `'flower'` et une couleur rose : la tempête verte du Druide faisait
